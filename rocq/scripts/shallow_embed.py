...
def name_to_rocq(name: str) -> str:
    ...
    # We do this replacement to get the name accepted in Rocq while still keeping a code
    # which is readable.
    return name.replace("usr$", "ᵤ").replace("$", "ₓ")


def names_to_rocq(as_pattern: bool, names: list[str]) -> str:
    if len(names) == 0:
        return "'tt" if as_pattern else "tt"

    if len(names) == 1:
        return name_to_rocq(names[0])

    quote = "'" if as_pattern else ""
    return \
        quote + "(" + \
        ', '.join(name_to_rocq(name) for name in names) + \
        ")"
...
def variable_name_to_rocq(variable_name) -> str:
    return name_to_rocq(variable_name_to_name(variable_name))


def variable_names_to_rocq(as_pattern: bool, variable_names: list) -> str:
    return names_to_rocq(
        as_pattern,
        [variable_name_to_name(variable_name) for variable_name in variable_names]
    )


def updated_vars_to_rocq(as_pattern: bool, updated_vars: set[str]) -> str:
    return names_to_rocq(as_pattern, sorted(updated_vars))
...
def block_to_rocq(
    ...
    if node_type == 'YulBlock':
        ...
        if return_variables is not None:
            suffix = "M.pure " + variable_names_to_rocq(False, return_variables)
        else:
            suffix = \
                "M.pure (BlockUnit." + return_mode + ", " + \
                updated_vars_to_rocq(False, updated_vars) + ")"
...
def lift_state_update(
    ...
    return \
        ...
        indent(
            "(fun " + updated_vars_to_rocq(True, local_updated_vars) + \
            " => " + updated_vars_to_rocq(False, global_updated_vars) + ")"
        ) + "\n" + \
        indent("(" + block + ")")


def statement_to_rocq(node) -> tuple[Callable[[set[str]], str], set[str], set[str]]:
    node_type = node.get('nodeType')

    if node_type == 'YulBlock':
        statement, statement_updated_vars = block_to_rocq(None, node)
        return (
            lambda final_updated_vars:
                "let_state~ " + \
                updated_vars_to_rocq(True, statement_updated_vars) + \
                " :=\n" + \
                indent(statement) + "\n" + \
        ...
        return (
            lambda _:
                "do~ [[ " + expression_to_rocq(node.get('expression')) + " ]] in",
            set(),
            set(),
        )
        ...
        condition = expression_to_rocq(node.get('condition'))
        post, post_updated_vars = block_to_rocq(None, node.get('post'))
        body, body_updated_vars = block_to_rocq(None, node.get('body'))
        updated_vars = post_updated_vars | body_updated_vars

        def fun_read_updated_vars(updated_vars: set[str]) -> str:
            return "(fun " + updated_vars_to_rocq(True, updated_vars) + " =>"

        return (
            lambda final_updated_vars:
                "let_state~ " + \
                updated_vars_to_rocq(True, updated_vars) + \
                " :=\n" + \
                indent(
                    "(* for loop *)\n" + \
                    "Shallow.for_\n" + \
...
def expression_to_rocq(node) -> str:
    ...
    if node_type == 'YulFunctionCall':
        func_name = variable_name_to_rocq(node['functionName'])
        args: list[str] = [expression_to_rocq(arg) for arg in node.get('arguments', [])]
        args_left = "~(|"
        args_right = "|)"
        return func_name + " " + \
    ...
    if node_type == 'YulIdentifier':
        return variable_name_to_rocq(node)
...
def function_definition_to_rocq(node) -> str:
    name = variable_name_to_rocq(node)
    param_names: list[str] = [
        variable_name_to_rocq(param)
    ...
    return \
        ...
        indent(
            "let~ '(_, " + updated_vars_to_rocq(False, body_updated_vars) + ") :=" + \
            "\n" + \
            (
                indent(
                    "let " + variable_names_to_rocq(True, returnVariables) + " := " + \
                    (
                        "0"
            ...
            indent(body) + "\n" + \
            "in\n" + \
            "M.pure " + variable_names_to_rocq(False, returnVariables)
        ) + "."
...
def top_level_to_rocq(node) -> str:
    node_type = node.get('nodeType')
    ...
    if node_type == 'YulBlock':
        ...
        functions = [
            function_definition_to_rocq(function)
        ...
        body = \
            ...
            indent(
                "let~ '(_, result) :=" + "\n" + \
                indent(block_to_rocq(None, node)[0]) + "\n" + \
                "in\n" + \
                "M.pure result"
            ) + "."
        return ("\n\n").join(functions + [body])
...
def object_to_rocq(node) -> str:
    node_type = node.get('nodeType')

    if node_type == 'YulObject':
        return \
            "Module " + node['name'] + ".\n" + \
            indent(object_to_rocq(node['code'])) + "\n" + \
            "".join(
                "\n" +
                indent(object_to_rocq(child)) + "\n"
        ...
        return top_level_to_rocq(node['block'])
...
def main():
    ...
    rocq_code = object_to_rocq(data)

    print("(* Generated by " + Path(__file__).name + " *)")
    print("Require Import RocqOfSolidity.RocqOfSolidity.")
    print("Require Import RocqOfSolidity.simulations.RocqOfSolidity.")
    print("Import Stdlib.")
    print()
    print(rocq_code)
