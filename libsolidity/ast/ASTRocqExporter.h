 ...
 */
class ASTRocqExporter: public ASTConstVisitor
{
public:
	...
	std::string toRocq(ASTNode const& _node);
	template<class T>
	std::string toRocqs(std::string const& separator, std::vector<ASTPointer<T>> const& _nodes)
	{
		std::string ret = "";
		bool isFirst = true;
...
private:
	...
	std::string sourceLocationsToRocq(std::vector<langutil::SourceLocation> const& _sourceLocations) const;
	static std::string namePathToString(std::vector<ASTString> const& _namePath);
	static std::string idOrNull(ASTNode const* _pt) { return _pt ? std::to_string(nodeId(*_pt)) : "null"; }
	std::string toRocqOrNull(ASTNode const* _node) { return _node ? toRocq(*_node) : ""; }
	std::string toRocqOrUnit(ASTNode const* _node) { return _node ? toRocq(*_node) : paren("Value.Tuple []"); }
	std::string toRocqOption(ASTNode const* _node)
	{
		if (_node)
		{
			m_withParens.push(true);
			std::string content = toRocq(*_node);
			m_withParens.pop();
	...
	std::string inlineAssemblyIdentifierToRocq(
	...
	static std::string typePointerToRocq(std::optional<FuncCallArguments> const& _tps);
